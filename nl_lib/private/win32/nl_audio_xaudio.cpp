...
class NL_XAudio2VoiceCallbacks : public IXAudio2VoiceCallback
{
    public: 
    ...
    {
        NL_UNUSED(pBufferContext); NL_UNUSED(Error); 
...
int init_audio_system(void)
{
    ...
    com_result = local_xaudio_system->xaudio_engine->CreateMasteringVoice( 
...
{   
    ...
    for (int i = 0; i < MAX_SOUND_BUFFERS; ++i)
    {
        ...
        {
            local_xaudio_system->xaudio_voices[i]->DestroyVoice();
        }
    }

    if (local_xaudio_system->xaudio_mastering_voice != 0)
    {
	    local_xaudio_system->xaudio_mastering_voice->DestroyVoice();
    }

    if (local_xaudio_system->xaudio_engine != 0)
	{
		local_xaudio_system->xaudio_engine->Release();
	}
...
internal_function unsigned int load_wav_sound(const char* filename)
{
    ...
    HRESULT source_result = local_xaudio_system->xaudio_engine->CreateSourceVoice
    (
	...
	source_result = local_xaudio_system->xaudio_voices[current_voice]->Start(0, XAUDIO2_COMMIT_NOW);
...
void set_master_volume(float volume)
{
    ...
    local_xaudio_system->xaudio_mastering_voice->SetVolume(volume, XAUDIO2_COMMIT_NOW);
}
