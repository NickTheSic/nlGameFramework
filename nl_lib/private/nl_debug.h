...
// TODO: Coloured logging, cus why not
...
# define NL_UNIMPLEMENTED_FUNC DO_ONCE(NL_LOG("NL_DEBUG: Unimplemented Function %s in %s", __FUNCTION__, __FILE__););
# define NL_DEPRECATED_FUNC(replace) DO_ONCE(NL_LOG("NL_DEBUG: Deprecated Function %s in %s, replace with %s", __FUNCTION__, __FILE__, replace););
# define NL_UNUSED(x) (void)(x); DO_ONCE(NL_LOG("NL_DEBUG: %s is not being used in %s", #x, __FUNCTION__););
