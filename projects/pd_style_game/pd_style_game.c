...
internal_function v2f get_movement_input(const GameControls controls, const float dt)
{
    ...
    speed.x = ((controls.MovementSpeed * is_key_held(controls.Right)) - (controls.MovementSpeed * is_key_held(controls.Left)));
    speed.y = ((controls.MovementSpeed * is_key_held(controls.Up)) - (controls.MovementSpeed * is_key_held(controls.Down)));
