...
void update_camera(nl_rr_camera* const camera, double dt)
{
    ...
    {
        camera->is_perspective = !camera->is_perspective;
    ...
    if (0 != move_delta)
    {
        camera->position.z +=  4.0f * -move_delta * dt;
    ...
    if (mouse_button_is_held(NL_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (0 != move_delta)
        {
            camera->position.z +=  220.0f * move_delta * dt;
            camera->transform_dirty = 1;
        }
    }
...
void setup_camera_matrices(nl_rr_camera* const camera)
{
    ...
    {
        NL_LOG("Orhto Camera created");
        create_orthographic_projection(&camera->proj_matrix, 0.0f, (float)camera->width, 0.0f, (float)camera->height, 0.0f, 1000.0f);
    } 
    else 
    {
        NL_LOG("Perspective Camera created");
        create_perspective_projection(&camera->proj_matrix, 90.f, camera->width/camera->height, 0.0f, 1000.0f);
    }
