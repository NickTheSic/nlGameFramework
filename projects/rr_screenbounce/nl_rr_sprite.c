...
void create_simple_rr_sprite(const char* filename, nl_rr_sprite* const rr_sprite)
{
    ...
    rr_sprite->scale = (v3f){(float)width, (float)height, 0.0f};
    rr_sprite->rotation = (v3f){0.0f,0.0f,0.0f};
    rr_sprite->position = (v3f){0.0f,0.0f,0.0f};

    create_srt_matrix(&rr_sprite->transform, rr_sprite->scale, rr_sprite->rotation, rr_sprite->position);
}
